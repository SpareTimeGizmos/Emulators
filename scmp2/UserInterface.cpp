//++
// UserInterface.cpp -> SCMP-II Emulator Specific User Interface code
//
// DESCRIPTION:
...
#include "INS8060opcodes.hpp"   // one line assembler and disassembler
...
const CCmdArgKeyword::KEYWORD CUI::m_keysSenseInputs[] = {
  {"SENSEA", CSCMP2::SENSEA},  {"SENSEB", CSCMP2::SENSEB},
  {"SIN",    CSCMP2::SIN},     {NULL, 0}
};
const CCmdArgKeyword::KEYWORD CUI::m_keysFlagOutputs[] = {
  {"FLAG0", CSCMP2::FLAG0},  {"FLAG1", CSCMP2::FLAG1},
  {"FLAG2", CSCMP2::FLAG2},  {"SOUT",  CSCMP2::SOUT},
  {NULL, 0}
};
...
size_t CUI::DoExamineInstruction (address_t nStart)
{
  ...
  size_t nCount = Disassemble2(g_pMemory, nStart, sCode);
  bOpcode = g_pMemory->UIread(nStart);
  address_t wNext = CSCMP2::INC12((address_t) nStart);
  if (nCount > 1) b2 = g_pMemory->UIread(wNext);
  wNext = CSCMP2::INC12(wNext);
...
bool CUI::DoRun (CCmdParser &cmd)
{
  ...
  if (m_argRunAddress.IsPresent()) {
    g_pCPU->SetRegister(CSCMP2::REG_PC, CSCMP2::DEC12(m_argRunAddress.GetNumber()));
  }
...
bool CUI::DoSetCPU (CCmdParser &cmd)
{
  ...
  // various options (e.g. stop on illegal opcode, set clock frequency, etc).
  //--
...
bool CUI::DoShowCPU (CCmdParser &cmd)
{
  ...
  double flMicroCycle = CSCMP2::CLOCKS_PER_MICROCYCLE / flCrystal;
  ...
  uint8_t bStatus = LOBYTE(g_pCPU->GetRegister(CSCMP2::REG_SR));
  string sStatus;
  if (ISSET(bStatus, CSCMP2::SR_CYL)) sStatus += "Carry/Link  ";
  if (ISSET(bStatus, CSCMP2::SR_OV )) sStatus += "Overflow  ";
  if (ISSET(bStatus, CSCMP2::SR_SB )) sStatus += "Sense B  ";
  if (ISSET(bStatus, CSCMP2::SR_SA )) sStatus += "Sense A  ";
  if (ISSET(bStatus, CSCMP2::SR_F2 )) sStatus += "Flag 2  ";
  if (ISSET(bStatus, CSCMP2::SR_F1 )) sStatus += "Flag 1  ";
  if (ISSET(bStatus, CSCMP2::SR_F0 )) sStatus += "Flag 0  ";
  if (ISSET(bStatus, CSCMP2::SR_IE )) sStatus += "Interrupt Enable ";
