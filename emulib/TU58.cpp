...
// 
// REVISION HISTORY:
// 18-JUL-20  RLA   New file.
// 16-JUL-22  RLA   Invent CVirtualConsole.
...
#include "TU58.hpp"             // declarations for this module


CTU58::CTU58 (uint8_t nUnits) : m_Images(nUnits)
{
  //++
  //--
  assert(nUnits > 0);
...
int32_t CTU58::RawRead (uint8_t *pabBuffer, size_t cbBuffer, uint32_t lTimeout)
{
  //++
  //   This is the CVirtualConsole routine that's called when the host wants
  // the TU58 to send him data.  It will see if we have anything to send and,
  // if we do, we'll pack it into the buffer and send it.  However it's quite
  // possible that we have nothing to say at the moment, in which case we
  // return an empty buffer and -1.
  // 
...
void CTU58::RawWrite (const char *pabBuffer, size_t cbBuffer)
{
  //++
  //   And this CVirtualConsole method is called when the host has data that
  // he wants to send to the TU58.  We just pull bytes out of the buffer and
...
void CTU58::SendSerialBreak (bool fBreak)
{
  //++
...
void CTU58::RxFromHost (uint8_t bData)
{
  //++
  ...
  //--
  int32_t nRet;  RSP_STATE OldState = m_nState;
  switch (m_nState) {
    ...
    case STA_INIT1:
      if (bData == RSP_F_INITIALIZE) m_nState = STA_INIT2;
      break;
    case STA_INIT2:
      if (bData == RSP_F_INITIALIZE) m_nState = STA_POWERUP;
      break;
    ...
    case STA_IDLE:
      if (bData == RSP_F_BOOTSTRAP) {
        ...
        RxPacketStart(bData);  m_nState = STA_RXCOMMAND;
      } else {
    ...
    case STA_WAITDATA:
      if (bData == RSP_F_DATA) {
        RxPacketStart(bData);  m_nState = STA_RXDATA;
      } else {
...
void CTU58::RxPacketStart (uint8_t bFlag)
{
  //++
...
int32_t CTU58::RxPacketData (uint8_t bData)
{
  //++
...
bool CTU58::TxToHost (uint8_t &bData)
{
  //++
  ...
  switch (m_nState) {
    ...
    case STA_POWERUP:
      bData = RSP_F_CONTINUE;  m_nState = STA_IDLE;  break;
    ...
    case STA_REQUESTDATA:
      bData = RSP_F_CONTINUE;  m_nState = STA_WAITDATA;  break;
    ...
    case STA_TXEND1:
      bData = m_RSPbuffer.bFlag;  m_nState = STA_TXEND2;  break;
    ...
    case STA_TXEND2:
      if (!TxPacketData(bData)) m_nState = STA_IDLE;
      break;
    ...
    case STA_TXDATA1:
      bData = m_RSPbuffer.bFlag;  m_nState = STA_TXDATA2;  break;
    case STA_TXDATA2:
      if (!TxPacketData(bData)) {
        if (ReadData()) m_nState = STA_TXDATA1;
      }
      break;
...
void CTU58::TxPacketStart (uint8_t bFlag, uint8_t bCount)
{
  //++
...
bool CTU58::TxPacketData (uint8_t &bData)
{
  //++
...
void CTU58::TxEndPacket (uint8_t bSuccess, uint16_t wCount)
{
  //++
  ...
  pEnd->wCount    = wCount;
  LOGF(DEBUG, "TU58 sending END, error=%d, count=%d", bSuccess, wCount);
  m_nState = STA_TXEND1;
}
...
bool CTU58::CheckUnit (bool fWrite)
{
  //++
...
void CTU58::DoCommand()
{
  ...
  LOGF(DEBUG, "TU58 command 0x%02X, unit=%d, block=%d, count=%d",
    m_RSPcommand.bOpcode, m_RSPcommand.bUnit, m_RSPcommand.wBlock, m_RSPcommand.wCount);
...
void CTU58::WriteData()
{
  ...
  LOGF(DEBUG, "TU58 received data, length=%d", m_RSPbuffer.bCount);
  ...
  if (m_cbTransfer >= m_RSPcommand.wCount) {
    LOGF(DEBUG, "TU58 write operation finished, %d bytes transferred", m_cbTransfer);
  ...
  if (fNeedWrite) {
    if (m_wCurrentBlock >= GetCapacity(nUnit)) {
      ...
      LOGF(DEBUG, "TU58 writing block %d to image", m_wCurrentBlock);
...
bool CTU58::ReadData()
{
  ...
  if (m_cbSector >= RSP_BLOCKSIZE) {
    if (m_wCurrentBlock >= GetCapacity(nUnit)) {
      ...
      LOGF(DEBUG, "TU58 reading image block %d", m_wCurrentBlock);
  ...
  LOGF(DEBUG, "TU58 sending data, length=%d", m_RSPbuffer.bCount);
  m_nState = STA_TXDATA1;  return true;
}
...
bool CTU58::Attach (uint8_t nUnit, const string &sFileName, bool fReadOnly, uint32_t lCapacity)
{
  //++
  ...
  // Success!
  LOGS(DEBUG, "TU58 unit " << nUnit << " attached to " << GetFileName(nUnit));
  LOGF(DEBUG, "TU58 unit %d capacity %ld blocks", nUnit, GetCapacity(nUnit));
  return true;
}

void CTU58::Detach (uint8_t nUnit)
{
  //++
  ...
  LOGS(DEBUG, "TU58 unit " << nUnit << " detached from " << GetFileName(nUnit));
  m_Images[nUnit]->Close();
}

void CTU58::DetachAll()
{
  //++
  // Detach ALL units ...
  //--
...
void CTU58::ShowDevice (ostringstream& ofs) const
{
  //++
  ...
  for (uint8_t i = 0;  i < GetUnits();  ++i) {
    ...
    if (IsAttached(i)) {
      ofs << GetFileName(i);
      if (IsReadOnly(i))  ofs << ", R/O";  else ofs << ", R/W";
  ...
  // Show the current controller state ...
  ofs << std::endl << "Current state: " << StateToString(m_nState);
  if (m_nState != STA_IDLE) {
    ofs << ", block " << m_wCurrentBlock << ", transfer " << m_cbTransfer;
  }
  ofs << std::endl << FormatString("Last command 0x%02X (%s), unit %d, block %d, count %d",
    m_RSPcommand.bOpcode, CommandToString(m_RSPcommand.bOpcode).c_str(),
    m_RSPcommand.bUnit, m_RSPcommand.wBlock, m_RSPcommand.wCount);
  ofs << std::endl;

  // Show the contents of the last/current packet ...
  ofs << std::endl;
  ofs << DumpBuffer("LAST PACKET", (const uint8_t *) &m_RSPbuffer, sizeof(m_RSPbuffer));
  ofs << FormatString("Checksum = 0x%02X%02X\n", m_bChecksumH, m_bChecksumL);
}
...
uint16_t CTU58::ComputeChecksum (const RSP_DATA* pPacket)
{
  //++
...
string CTU58::StateToString (RSP_STATE nState)
{
  //++
  ...
  switch (nState) {
    ...
    default: return FormatString("%d", nState);
  }
}

string CTU58::CommandToString (uint8_t bCommand)
{
  //++
  // Convert an RSP command byte to a string for debugging ...
  //--
  switch (bCommand) {
    case RSP_O_NOP:         return "NOP";
    case RSP_O_INITIALIZE:  return "INIT";
    case RSP_O_READ:        return "READ";
    case RSP_O_WRITE:       return "WRITE";
    case RSP_O_POSITION:    return "POSITION";
    case RSP_O_DIAGNOSE:    return "DIAGNOSE";
    case RSP_O_GETSTATUS:   return "GET STATUS";
    case RSP_O_SETSTATUS:   return "SET STATUS";
    case RSP_O_END:         return "END";
    default:                return "???";
  }
}
